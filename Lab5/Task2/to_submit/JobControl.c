...
job* initializeJob(char* cmd){
    job* jb = (job*)malloc(sizeof(char)+sizeof(int)+sizeof(pid_t)+sizeof(int)+sizeof(void*)*2);
    jb->cmd=strdup(cmd);
    jb->idx = 1;
    jb->pgid = 0;
    jb->status = RUNNING;
    jb->tmodes = (struct termios*)malloc(sizeof(struct termios));
    jb->next = 0;
    return jb;
}
...
job* findJobByIndex(job* job_list, int idx){
  int i;
  for(i=1; i<idx && job_list; i++){
      job_list = job_list->next;
  }
  if(!job_list){
      fprintf(stderr, "%s", "No such job exists in the given list");
  }
  return job_list;
}
...
void updateJobList(job **job_list, int remove_done_jobs){
    ...
    while(curr_job){
        job* tmp = curr_job->next;
        if(waitpid(curr_job->pgid, &status, WNOHANG)){
            if(remove_done_jobs){
