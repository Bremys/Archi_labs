...
void print_ent(int d_flag, dirent* entp){
...
int main (int argc , char* argv[], char* envp[])
{
    ...
    dirent* entp = (dirent*) buf;
    
    while((int)entp<(int)(buf+count)){
        if(suffix == '\0' || entp->d_name[strlen(entp->d_name)-1]==suffix){
        ...
        entp = (dirent*) (((char*)entp)+entp->d_reclen);
    }
    return 0;
}
